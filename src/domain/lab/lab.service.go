...
func (s *labService) Get(ctx *gin.Context, labFilter LabFilter, authUser auth.AuthenticatedUser) ([]Lab, error) {
	var (
		labs []Lab
		err  error
	)
...
func (s *labService) GetByUuid(ctx *gin.Context, labId string, authUser auth.AuthenticatedUser) (*Lab, error) {
	var (
		lab *Lab
		err error
	)
...
func (s *labService) SetRuntimeInfo(runtimeInfo RuntimeInfo) {
